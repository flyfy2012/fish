...
public class Menu extends Model<Menu> {
	...
	public List<Menu> getAllowedAccessMenus(Integer userId) {
		...
		sqlBuffer.append(" select m.id,m.name,m.parent_menu,m.description,m.ref,m.orderby from sec_menu m ");
		sqlBuffer.append(" where not exists (select re.id from sec_resource re where re.menu = m.id)" );
		sqlBuffer.append(" union ");
		//获取Sec_Resource表中已关联且未设置权限的菜单列表
		sqlBuffer.append(" select m.id,m.name,m.parent_menu,re.source as description,m.ref,m.orderby from sec_resource re ");
		...
		sqlBuffer.append(" select m.id,m.name,m.parent_menu,re.source as description,m.ref,m.orderby from sec_user u ");
	...
	public List<Menu> getByParent_menu(Integer parentId) {
		String sql = "select parent_menu,name,id from sec_menu";
		if(parentId != null && parentId >= 0) {
			sql += " where parent_menu =" + parentId;
		}
		return find(sql);
	}
	
	
	public List<Menu> getById(Integer id) {
		
			return find("select parent_menu,name,id from sec_menu where id="+id);
		
	}
	public List<Menu> getForEdit() {
		String sql = "select name,id from sec_menu";
